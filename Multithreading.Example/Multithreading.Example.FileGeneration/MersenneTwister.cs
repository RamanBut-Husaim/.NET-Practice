...
namespace Multithreading.Example.FileGeneration
{
  ...
  public sealed class MersenneTwister : Random
  {
    ...
    private const int MaxRandInt = 0x7fffffff;
    ...
    private uint[] mag01 = { 0x0U, MatrixA };
    ...
    public new int Next(int minValue, int maxValue)
    {
      ...
      return (int)(Math.Floor((maxValue - minValue + 1) * this.genrand_real2() + minValue));
    }
    ...
    private void init_by_array(uint[] init_key, uint key_length)
    {
      ...
      for (; k > 0; k--)
      {
        this.mt[i] = (uint)((uint)(this.mt[i] ^ ((this.mt[i - 1] ^ (this.mt[i - 1] >> 30)) * 1664525U)) + init_key[j] + j); /* non linear */
      ...
      for (k = N - 1; k > 0; k--)
      {
        this.mt[i] = (uint)((uint)(this.mt[i] ^ ((this.mt[i - 1] ^ (this.mt[i - 1] >> 30)) * 1566083941U)) - i); /* non linear */
        this.mt[i] &= 0xffffffffU; // for WORDSIZE > 32 machines
        i++;
        if (i >= N) { this.mt[0] = this.mt[N - 1]; i = 1; }
      }
    ...
    {
      return (((double)this.genrand_int32()) + 0.5) * (1.0 / 4294967296.0);
      // divided by 2^32
    }
